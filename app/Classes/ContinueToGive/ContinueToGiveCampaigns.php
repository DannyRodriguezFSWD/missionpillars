...
use App\Constants;
/**
 * Description of ContinueToGiveCampaigns
 *
 * @author josemiguel
 */
class ContinueToGiveCampaigns extends ContinueToGiveIntegration implements ContinueToGiveRunnable{
    use PurposeTrait, CampaignTrait, ContactTrait, AlternativeIdTrait;
    ...
    public function call($params = []) {
        $uri = $this->getUri() . 'campaigns?token=' . $this->getToken() .'&';
    ...
    public function store($json) {
        if ($json) {
            ...
            foreach ($dataset as $data) {
                    //\Log::info(array_get($data,'contact.sub_type'));
                    $contact = null;
                    if (array_get($data,'contact.sub_type') != 'organizations') {
                    ...
                    if(array_get($data, 'sub_type') === Constants::CHART_OF_ACCOUNT_SUBTYPE_GIVINGPAGES){
                        $parent = $this->alternativeIdRetrieve(array_get($data, 'for_pageid', 0), Purpose::class);
    ...
    /**
     * Helps to migrate the ctg.page.subtype to mp.tenant.type for existing existing tenants
     * @param Array $params the ctg.page.id (retrieved from mp.alt_id)
     */
    public function getTenantType($params = []) {
        $json = $this->call($params);
        if(!empty($json) && !is_null($json)){
            $dataset = array_get($json, 'data', []);
            foreach ($dataset as $data) {
                $tenant = auth()->user()->tenant;
                array_set($tenant, 'type', array_get($data, 'sub_type'));
                $tenant->update();
            }
        }
    }
