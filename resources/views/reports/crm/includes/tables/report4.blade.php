        ...
        <thead>
            <tr>
                @foreach ($givers as $giver)
                    @if($loop->first)
                        <th><i class="fa fa-sort" aria-hidden="true"></i> Fundraiser</th>
            ...
            <tbody>
                <tr class="table-success">
                    ...
                    @foreach(array_get($sum, 'ranges', []) as $range)
                        @if(count($range) > 0)
                        <td class="text-right campaigns_total_column">{{ array_get($range, '0.total_transactions') }}</td>
                        <td class="text-right campaigns_total_column">${{ number_format(array_get($range, '1.total_amount'), 2) }}</td>
                        @else
