...
<p>
    Thank you very much for your gift of ${{ array_get($split, 'amount') }} toward your{{ array_get($pledge, 'template.is_recurring') ? ' recurring ':' ' }}pledge of ${{ array_get($pledge, 'amount') }} to {{ array_get($campaign, 'id', 1) > 1 ? array_get($campaign, 'name') : array_get($chart, 'name') }}, with a promised pay date of {{ humanReadableDate(array_get($pledge, 'template.billing_start_date')) }}.
</p>
<p>
