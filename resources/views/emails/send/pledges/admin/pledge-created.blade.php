...
<p>
    {{ array_get($contact, 'first_name') }} {{ array_get($contact, 'last_name') }} has made a{{ array_get($template, 'is_recurring') ? ' recurring ':' ' }}pledge of ${{ array_get($split, 'amount') }} to {{ array_get($campaign, 'id', 1) > 1 ? array_get($campaign, 'name') : array_get($chart, 'name') }}, with a promised pay date of {{ humanReadableDate(array_get($pledge, 'template.billing_start_date')) }}.
</p>
<p>
...
<p>
    They can fulfill this pledge by clicking here:
