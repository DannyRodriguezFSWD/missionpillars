    ...
    <div class="card-body">
        <div class="row hide-on-print">
            <div class="col-md-12 text-right">
                <button class="btn btn-secondary" type="submit" @click="$refs.export_form.submit()">
        ...
        <div class="row mt-3 hide-on-print" v-if="filters === 'true' || range == 7">
            <div class="col-md-2">
                <input type="button" value="Apply" class='btn btn-success' @click="query()">
            </div>
        </div>
        ...
        <div id="printReport">
            <div class="row mt-5">
                <div class="col-md-12 text-center">
                    ...
                    <h5>Income Statement {{ reportPeriod }}</h5>
                </div>
            </div>
                ...
                <table class="table borderless">
                    ...
                    <tbody>
                        ...
                        <template v-for="income in report.income">
                            <tr v-for="account in income.accounts" :key="account.account_id" v-if="(account.amount !== null && account.amount !== 0) || (account.is_sub_account !== 1 && account.amount_with_sub !== null && account.amount_with_sub !== 0)">
                                <td>&nbsp;</td>
                                <td>{{ account.account_number }}</td>
                                <td>{{ account.account_name }}</td>
                                <td class="text-right">
                                    <span v-if="account.amount < 0">
                                        (${{ account.amount == null ? '0.00' : account.amount.toFixed(2) }})
                                    </span>
                                    <span v-else>
                                        ${{ account.amount == null ? '0.00' : account.amount.toFixed(2) }}
                                    </span>
                        ...
                        <template v-for="expense in report.expense">
                            <tr v-for="account in expense.accounts" :key="account.account_id" v-if="(account.amount !== null && account.amount !== 0) || (account.is_sub_account !== 1 && account.amount_with_sub !== null && account.amount_with_sub !== 0)">
                                <td>&nbsp;</td>
                                <td><span v-if="account.is_sub_account == 1" class="mr-4"></span>{{ account.account_number }}</td>
                                <td><span v-if="account.is_sub_account == 1" class="mr-4"></span>{{ account.account_name }}</td>
                                <td class="text-right">
                                    <span v-if="account.amount < 0">
                                        (${{ account.amount == null ? '0.00' : account.amount.toFixed(2) }})
                                    </span>
                            ...
                            <tr>
                                <td><strong>Total {{ expense.group.group_name }} Amount</strong></td>
                                <td>&nbsp;</td>
                                <td>&nbsp;</td>
                                <td class="totals-number">&nbsp;</td>
                                <td class="text-right totals-number">
        ...
        <form class="d-none" ref="export_form" method="POST" :action="'/accounting/reports/income-statement/'+new Date().getTime()+'/export'" accept-charset="UTF-8">
            ...
            <input name="funds" type="hidden" :value="JSON.stringify(filters_fund_array)">
        </form>
    ...
    methods: {
        ...
        setReportPeriod(range, data) {
            ...
            if (range == 1) {
                ...
                this.reportPeriod = '- ' + monthNames[start.getMonth()] + ' ' + start.getFullYear();
            } else if (range == 3 || range == 4) {
                this.reportPeriod = '- ' + monthNames[start.getMonth()] + ' - ' + monthNames[end.getMonth()] + ' ' + end.getFullYear();
            } else if (range == 5 || range == 6) {
                this.reportPeriod = '- ' + start.getFullYear();
            } else if (range == 7) {
